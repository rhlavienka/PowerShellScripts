...
.DESCRIPTION
    Parses the most recent Exchange IMAP4 protocol logs and counts client LOGIN /
    AUTHENTICATE commands, grouped by client source IP address and the server
    ...
    IMAP4 protocol logging is disabled by default on Exchange Server and must be
...
.EXAMPLE
    ...
    Processes the 20 most recent IMAP4 protocol logs on the local Exchange
...
# Path to IMAP4 protocol logs
$LogPath = "C:\Program Files\Microsoft\Exchange Server\V15\Logging\Imap4"
...
$Results = $AllRecords |
    Where-Object {
        ...
        $_.Data -match '(?i)\bLOGIN\b|\bAUTHENTICATE\b'
    } |
    Where-Object {
        $_.ClientIP
    } |
